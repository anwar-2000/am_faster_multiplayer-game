...
function RoomGame({autoStart,roomId,username,id,text,category,difficulty}) {
  ...
  const handleUserInput = (event) => {
    ...
    if (mismatchIndex === -1 && input.length === text.length) {
      ...
      //broadcast results
      socket.emit("gameEnded",({challengeId : id , username,roomId,finishedIn : gameStats.elapsedTime , mistakes : gameStats.all_mistakes}))
  ...
  useEffect(() => {
    autoStart && startButton.current.click()
    socket.on("winnerAnnouncement",({winner,mistakes,time})=>{
